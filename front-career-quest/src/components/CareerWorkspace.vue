...
<template>
  <div class="page-wrap">
    <section class="hero-row"><div><p class="eyebrow">{{ t('pages.snapshot', { date: date(snapshotDate) }) }}</p><h1>{{ page === 'overview' ? t('pages.hello', { name: employee.full_name.split(' ')[0] }) : t(page) }}</h1><p class="hero-subtitle">{{ t(`pages.intro.${page}`) }}</p></div><button v-if="['overview', 'path'].includes(page)" class="outline-button" @click="downloadReport">{{ t('downloadReport') }} ↓</button></section>
    <p v-if="storageError" class="notice" role="status">{{ t('pages.storageError') }}</p>

    <template v-if="page === 'overview'">
      <section class="stats-grid">
        <article class="stat-card"><div class="stat-label">{{ t('currentGrade') }}</div><div class="stat-value">{{ employee.grade }}</div><div class="stat-note">{{ employee.role }}</div></article>
        <article class="stat-card"><div class="stat-label">{{ t('progressToGoal') }}</div><div class="stat-value">{{ employee.career_goal ? `${progress}%` : '—' }}</div><div class="stat-note">{{ employee.career_goal ? `${targetRole} · ${targetGrade}` : t('pages.noGoal') }}</div></article>
        <article class="stat-card"><div class="stat-label">{{ t('strongSkill') }}</div><div class="stat-value skill-stat">{{ skillName(strongest[0]) }}</div><div class="stat-note">{{ strongest[1] }} / 5</div></article>
        <article class="stat-card"><div class="stat-label">{{ t('pages.completed') }}</div><div class="stat-value">{{ personalHistory.filter(row => row.status === 'completed').length }}</div><div class="stat-note">{{ t('pages.historyPeriod') }}</div></article>
      </section>
      <section class="main-grid"><article class="panel"><p class="section-kicker">{{ t('development') }}</p><h2>{{ t('pages.prioritySkills') }}</h2><p class="muted">{{ t('pages.reviewDate', { date: date(employee.last_review_date) }) }}</p><div class="skill-list"><div v-for="skill in gaps.slice(0, 4)" :key="skill.skill_id"><div class="skill-info"><strong>{{ skillName(skill.skill_id) }}</strong><b>{{ skill.current }} / {{ skill.target }}</b></div><div class="bar"><span class="teal" :style="{ width: `${skill.current / 5 * 100}%` }"></span><i :style="{ left: `${skill.target / 5 * 100}%` }"></i></div></div><p v-if="!gaps.length" class="muted">{{ t('pages.noGaps') }}</p></div><a class="text-button page-link" href="#path">{{ t('openPath') }} →</a></article><article class="panel goal-panel"><p class="section-kicker">{{ t('careerGoal') }}</p><h2>{{ employee.career_goal ? `${targetRole} · ${targetGrade}` : t('pages.noGoal') }}</h2><p class="goal-copy">{{ t('pages.progressExplanation') }}</p><div v-if="employee.career_goal" class="goal-bar"><span :style="{ width: `${progress}%` }"></span></div><p class="muted">{{ t('pages.plannedCount', { count: plannedEvents.length }) }}</p><a class="dark-button page-link" href="#path">{{ t('openPath') }} →</a></article></section>
      <section class="panel"><div class="panel-heading"><div><p class="section-kicker">{{ t('nextStep') }}</p><h2>{{ t('recommendations') }}</h2></div><a href="#catalog" class="text-button">{{ t('catalog') }} →</a></div><p class="muted">{{ t('pages.recommendationNote') }}</p><div class="activity-grid spaced"><EventCard v-for="event in recommendations.slice(0, 3)" :key="event.event_id" :event="event" :planned="planIds.includes(event.event_id)" @details="showEvent" /></div><p v-if="!recommendations.length" class="empty-state">{{ t('pages.noRecommendations') }}</p></section>
    </template>
