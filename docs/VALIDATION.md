...
После интеграции backend повторно проверено: `pytest -q` — 76 passed; `scripts/check_dataset.py` — полный исходный набор; `scripts/check_backend.py` — завершение без повторного прироста, импорт, права доступа и сохранение после перезапуска в режиме `fallback`. Completion, Import и HR API теперь работают. Живой LLM и браузерный проход общего `main` не проверены.

В ветке Саята для `frontend/` отдельно проверены 41 клиентский тест и production-сборка. Это не является проверкой общего `main`, пока фронтенд не перенесён выборочно.

## Первоначальная проверка каркаса
...
В текущем main первоначальные тесты на 501 заменены acceptance-тестами.
